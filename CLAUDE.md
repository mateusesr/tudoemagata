...
Correções pós-QA manual do usuário (mesma sessão, após primeiro teste no navegador):
- **Checkout quebrava com "Attempt to read property addresses on null"**: usuários criados fora do fluxo Breeze/Google (ex: `make:filament-user`) não tinham `Customer`. `CheckoutController` agora usa `firstOrCreate` defensivo em vez de assumir que `$user->customer` sempre existe.
- **Não havia UI para cadastrar endereço**: só existia o model `CustomerAddress`, sem tela nenhuma — o checkout ficava travado em "cadastre um endereço" sem meio de fazer isso. Adicionado formulário inline no checkout (`POST /checkout/enderecos`, `CheckoutController::storeAddress`).
- **Carrinho "não somava em tempo real"**: o backend sempre esteve correto; o problema era que erros de validação (ex: quantidade acima do estoque) falhavam silenciosamente sem feedback visual. Adicionado `itemErrors` no Alpine do carrinho.
- **Login sempre caía em `/dashboard` em branco em vez de voltar pra página anterior**: o link "Entrar" no header ia direto pra `/login` sem popular a "intended URL" do Laravel (isso só acontece automaticamente quando o middleware `auth` intercepta uma rota protegida). Corrigido propagando `?redirect_to=` pela URL atual → guardado em `session('url.intended')` nos 3 pontos de entrada (`AuthenticatedSessionController::create`, `RegisteredUserController::store`, `GoogleAuthController::redirect`) → login/registro/Google agora usam `redirect()->intended()`/`redirect_to` corretamente nos três fluxos.
- **Gap adicional descoberto no caminho**: a tela de login não tinha link para "criar conta" nem botão de login com Google (que já existia no backend desde a Fase 0, mas nunca foi exposto na UI). Adicionado em `resources/views/auth/login.blade.php`/`register.blade.php`, com textos também traduzidos para pt_BR (o Breeze scaffold usa `__('Email')` etc. sem tradução real publicada — são strings literais em inglês).
- Falta ainda: área do cliente (histórico de pedidos — hoje só existe o model, sem UI de listagem/detalhe), Filament Resources de estoque/vendas externas/configurações, **credenciais reais de sandbox do Mercado Pago e do Melhor Envio** (ver "Dependências externas" abaixo — necessárias para o usuário continuar testando o fluxo de pagamento/frete de verdade, hoje só validado com `Http::fake()`).
